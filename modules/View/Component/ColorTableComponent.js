export default class ColorTableComponent {
    ...
    onChange(change, state) {
        ...
        const relevantChildren = Array.from(this.#element.querySelectorAll(".color-cell"))
        const uniqueColors = this.#findColors(state.marks);
        const existingColors = relevantChildren.map(child => child.dataset.color);

        const existsButShouldnt = existingColors.filter(existingColor => !uniqueColors.includes(existingColor))
        const doesntYetExist = uniqueColors.filter(uniqueColor => !existingColors.includes(uniqueColor))
        // remove existsButShouldnt
        relevantChildren.filter(child => existsButShouldnt.includes(child.dataset.color)).forEach(child => child.remove())
        // add doesntYetExist
    ...
    #buildColorCell(color) {
        ...
        cell.addEventListener("click", () => this.#controller.onStrokeColorChanged(color))
        return cell;
    }
}
